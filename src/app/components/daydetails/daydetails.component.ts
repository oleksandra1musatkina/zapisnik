...
export class DaydetailsComponent implements OnInit {
  ...
  ngOnInit() {
    this.userdetailservice.getusers().subscribe(data => {
      this.user = data;
    });
    ...
    this.imgSrc =
      "https://cdn4.iconfinder.com/data/icons/ionicons/512/icon-ios7-cloud-upload-outline-512.png";
  }
  ...
  onSubmit() {
    ...
    if (this.daydetails.valid) {
      ...
      const date = new DatePipe("en-US").transform(
        this.daydetails.value.date,
        "ddMMyyyy"
      );
      this.daydetails.value.date = new DatePipe("en-US").transform(
        this.daydetails.value.date,
        "dd/MM/yyyy"
      );
